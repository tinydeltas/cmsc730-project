...
from ml.siamese import * 
from data.generate import DatasetLoader
import data.params

run = datetime.now().strftime("%m_%d_%Y_%H:%M:%S")
run_path = os.path.join(data.params.run_directory, run)

ways = np.arange(1, len(params.default_gestures) + 1, 1)
...
def main(): 
    ...
    for spectrogram_type in params.input_image_types: 
        ...
        dl.generate(params.default_gestures)
        ...
        print("Saving model to ", loader.model_path)
        model.save(loader.model_path)
